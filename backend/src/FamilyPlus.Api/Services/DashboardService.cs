...
public sealed class DashboardService(FinanceDbContext db, RecurrenceService recurrences, BudgetService budgets, Func<DateTimeOffset>? utcNow = null)
{
    public async Task<DashboardSummaryResponse> GetSummaryAsync(DashboardQueryRequest request)
    {
        ...
        var evolution = BuildEvolution(realTransactions, scope.Purchases.Where(x => !request.MembroId.HasValue || x.MembroId == request.MembroId).ToList(), purchaseRefunds, accounts, today);
    ...
    private static IReadOnlyList<DashboardMonthlyResponse> BuildEvolution(IReadOnlyList<Transacao> transactions, IReadOnlyList<CompraCartao> purchases, IReadOnlyDictionary<Guid, long> refunds, IReadOnlyList<Conta> accounts, DateTimeOffset today)
    {
        var start = new DateTimeOffset(today.Year, today.Month, 1, 12, 0, 0, TimeSpan.Zero).AddMonths(-5); var result = new List<DashboardMonthlyResponse>();
        for (var index = 0; index < 6; index++) { var month = start.AddMonths(index); var next = month.AddMonths(1); var rows = transactions.Where(x => x.Status == StatusTransacao.EFETIVADA && x.Origem != OrigemTransacao.PAGAMENTO_FATURA && (x.Tipo is TipoTransacao.RECEITA or TipoTransacao.DESPESA) && InRange(x.DataMovimentacao, month, next.AddDays(-1))).ToList(); var monthPurchases = purchases.Where(x => x.Status != StatusCompraCartao.CANCELADA && InRange(x.DataCompra, month, next.AddDays(-1))).Sum(x => Math.Max(0, x.ValorTotalCentavos - refunds.GetValueOrDefault(x.Id))); var income = rows.Where(x => x.Tipo == TipoTransacao.RECEITA).Sum(x => x.ValorCentavos); var expense = rows.Where(x => x.Tipo == TipoTransacao.DESPESA).Sum(x => x.ValorCentavos) + monthPurchases; var balance = accounts.Sum(x => x.SaldoInicialCentavos) + transactions.Where(x => x.Status == StatusTransacao.EFETIVADA && x.DataMovimentacao < next).Sum(x => FinanceCalculator.Delta(x.Tipo, x.ValorCentavos)); result.Add(new DashboardMonthlyResponse(month, income, expense, income - expense, balance)); }
        return result;
    }
